                        ...
                        foreach ($rows as $r) {
                            if ( $r->post_target == '84' ){ ?>
                                <div class="items">
                                    <div class="col-md-1">
                                        <div class="ev1">
                                          <p class="da1"> <?php  echo date('M',strtotime($r->time_created)) ;  ?> </p>
                                          <p class="da2"> <?php  echo date('j',strtotime($r->time_created)) ;  ?> </p>
                                          <p class="da3"> <?php  echo date('Y',strtotime($r->time_created)) ;  ?> </p>
                                        </div>
                                    </div>
                                    <div <?php if($this->session->userdata('lang') == 'arabic'){ echo 'dir="rtl"'; } ?> class="new col-md-11">
                                        <div style="font-size: 26px; font-weight: bold;">
                                        <?php										
                                        if($this->session->userdata('lang') == 'english' || $this->session->userdata('lang') == NULL || $this->session->userdata('lang') == "" ){ echo $r->post_title.'<br>'.$r->post_desc; }
                                        if($this->session->userdata('lang') == 'arabic'){ echo $r->post_title_ar.'<br>'.$r->post_desc_ar; } ?>
                                        </div>
                                    </div>
                                </div>
                    <?php   } } ?>
...
<?php
	foreach ($rows as $r) {
       if ( $r->post_target == '84' ){ ?>
			<div id="iframemodal<?php echo $r->post_id; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-lg">
					<div class="modal-content">
						<div class="modal-header">
							<?php 
                            if($this->session->userdata('lang') == 'english' || $this->session->userdata('lang') == NULL || $this->session->userdata('lang') == "" ) { echo $r->post_title; }
                            if($this->session->userdata('lang') == 'arabic'){ echo $r->post_title_ar; }
							?>
							<button type="button" class="close" data-dismiss="modal">&times;</button>
							<br>
						</div>
						<div class="modal-body"  style="min-height: 500px; max-height: 800px;">
							<?php 
							if($this->session->userdata('lang') == 'english' || $this->session->userdata('lang') == NULL || $this->session->userdata('lang') == "" ){ preg_match('/^<a.*?href=(["\'])(.*?)\1.*$/', $r->post_desc, $post_url); }
							if($this->session->userdata('lang') == 'arabic'){ preg_match('/^<a.*?href=(["\'])(.*?)\1.*$/', $r->post_desc_ar, $post_url); }
							?>
							<iframe width="100%" style="min-height: 400px; max-height: 700px;" src="<?php print_r($post_url[2]); ?>" sandbox></iframe>
						</div>
					</div>
				</div>
			</div>
    ...
    <div class=" container">
        <div class="row">
            ...
            <div class="col-md-6 contact-form">
                ...
                <?php echo form_open('home/home_page/send_message'); ?>
